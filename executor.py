...
class Communicator:
    ...
    def listen_for(self, packet_id=0, timeout=4, payload_pattern=None, source_request=None):
        ...
        while time_left > 0:
            ...
            if raw_packet != b'':
                ...
                if response.id == packet_id and response.message_type != icmp.Types.EchoRequest.type_id:
                    ...
                    if payload_matched and not loop:
    ...
    def listen_for_new(self, timeout=4, payload_pattern=None, source_request=None, loop=TIMEIT):
        """Listens for a packet of a given id for a given timeout
        ...
        response = icmp.ICMP()
        found = False
        till = int(time.time()) + loop
        time_list = []
        start = 0
        end = 0
        while till-1 > int(time.time()):
            found = False
            if not start:
                start = int(time.time())
            # Keep listening until a packet arrives
            raw_packet, source_socket, time_left = self.socket.receive(time.time()+2)
            # If we actually received something
            if raw_packet != b'':
                response.unpack(raw_packet)
                if response.message_type != icmp.Types.EchoRequest.type_id:
                    found = True
                    #print('found')
                    end = int(time.time())
                    if end - start > 1:
        ...
        if start and int(time.time()) - start > 1:
            time_list.append((start, int(time.time())))
        #print(time_list)
        return time_list
    ...
    def send_in_loop(self):
        self.responses.clear()
        loop = TIMEIT
        till = int(time.time()) + loop
        while till-1 > int(time.time()):
            identifier = self.seed_id + loop
            seq = 1
            for payload in self.provider:
                icmp_out = self.send_ping(identifier, seq, payload)
                seq = self.increase_seq(seq)
                time.sleep(1)
            #print(till, int(time.time()))
    def run(self, match_payloads=False):
        import threading
        send_thread = threading.Thread(target=self.send_in_loop)
        send_thread.start()
        rcv_thread = threading.Thread(target=self.listen_for_new)
        rcv_thread.start()
        time.sleep(TIMEIT+1)
        send_thread.join()
        rcv_thread.join()

    def run_old(self, match_payloads=False):
